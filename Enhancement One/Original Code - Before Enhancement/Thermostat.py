...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        # Call utility to update indicator lights based on current temp vs. setpoint.
        self.updateLights()
    ...
    def on_exit_heat(self):
        # Ensure the Red LED is turned off when leaving the heating state.
        redLight.off()
    ...
    def on_exit_cool(self):
        # Ensure the Blue LED is turned off when leaving the cooling state.
        blueLight.off()
    ...
    def on_enter_off(self):
        # Turn off both Red and Blue LEDs when entering the 'off' state.
        redLight.off()
        blueLight.off()
    ...
    def processTempStateButton(self):
        ...
        # Trigger the state machine's cycle event to transition state (off -> heat -> cool -> off).
        self.cycle()
        self.updateLights()
    ...
    def processTempIncButton(self):
        ...
        # Increment the temperature set point by one degree Fahrenheit.
    ...
    def processTempDecButton(self):
        ...
        # Decrement the temperature set point by one degree Fahrenheit.
        self.setPoint = self.setPoint - 1
        self.updateLights()
    ...
    def updateLights(self):
        ...
        # Logic for indicator lights based on state and temperature
        if self.current_state.id == 'heat':
            if temp < self.setPoint:
                # Heating is required: Red LED fades (blinks).
                redLight.blink(on_time=1, off_time=1, fade_in_time=0.5, fade_out_time=0.5)
            else:
                # Target achieved: Red LED is solid on.
                redLight.on()
        elif self.current_state.id == 'cool':
            if temp > self.setPoint:
                # Cooling is required: Blue LED fades (blinks).
                blueLight.blink(on_time=1, off_time=1, fade_in_time=0.5, fade_out_time=0.5)
            else:
                # Target achieved: Blue LED is solid on.
                blueLight.on()
    ...
    def setupSerialOutput(self):
        ...
        # Format output as a comma-delimited string: State, Current Temp, Setpoint.
        output = f"{self.current_state.id}, {current_temp_f:.2f}, {self.setPoint}"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            # Format and assign the current date and time to the first line of the LCD.
            ...
            if(altCounter < 6):
                ...
                # Display the current measured temperature (rounded to one decimal).
                lcd_line_2 = f"Temp: {current_temp_f:.1f}"
    
                altCounter = altCounter + 1
            else:
                # Display the current state and the temperature setpoint.
                lcd_line_2 = f"State: {self.current_state.id}\nSet: {self.setPoint}"
            ...
            if((counter % 30) == 0):
                ...
                # Send the state information over UART and append a newline character.
